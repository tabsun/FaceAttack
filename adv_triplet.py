...
from losses.face_losses import arcface_loss, batch_arcface_loss, triplet_loss
...
if __name__ == '__main__':
    ...
    emd_g_dict = dict()
    count = 0
    for image_name, emd_c_, emd_d_, emd_a_, emd_f_, emd_g_ in zip(df_c.image_name, df_c.id_embedding, df_d.id_embedding, df_a.id_embedding, df_f.id_embedding, df_g.id_embedding):
        count += 1
        print(count)
    ...
    for image_name in df_c.image_name:
        ...
        for com_image_name in emd_c_dict.keys():
            ...
            dist = dist_c + dist_a +dist_f# + dist_g
    ...
    #single_gray_net_input = tf.expand_dims(0.299*net_input[:,:,:,0] + 0.587*net_input[:,:,:,1] + 0.114*net_input[:,:,:,2], -1)
    #gray_net_input = tf.concat([single_gray_net_input, single_gray_net_input, single_gray_net_input], axis=-1)

    #mean = 1.0
    #std = 4.0
    #d = tf.distributions.Normal(mean, std)
    #vals = d.prob(tf.range(start=-5, limit=5+1, dtype=tf.float32))
    #gauss_kernel = tf.einsum('i,j->ij', vals, vals)
    #gauss_kernel = (gauss_kernel / tf.reduce_sum(gauss_kernel))[:,:, tf.newaxis, tf.newaxis]
    #gauss_net_input = tf.nn.conv2d(net_input, gauss_kernel, strides=[1,1,1,1], padding="SAME")
    ...
    total_loss = trip_loss_c + trip_loss_a + trip_loss_f# + trip_loss_g 
    delta_grad = tf.gradients(total_loss, input_deltas)[0]
    #delta_grad_c = tf.gradients(trip_loss_c, input_deltas)[0]
    #delta_grad_d = tf.gradients(trip_loss_d, input_deltas)[0]
    #delta_grad_a = tf.gradients(trip_loss_a, input_deltas)[0]
    #delta_grad_f = tf.gradients(trip_loss_f, input_deltas)[0]
    ...
    norm_cum = 0
    cum_num = 0
    for image_name, cur_attack_image, cur_start_delta in zip(df.ImageName[args.start_pos:args.start_pos+100], 
            original_images[args.start_pos:args.start_pos+100], 
            start_deltas[args.start_pos:args.start_pos+100]):
        ...
        for target_image_name in target_image_names:
            ...
            lost = False
            zero_passed = False
            prev_loss_c = float('inf')
            prev_loss_a = float('inf')
            prev_loss_f = float('inf')
            prev_loss_d = float('inf')
            prev_loss_g = float('inf')
            for i in range(0, steps):
                loss_c, loss_d, loss_a, loss_f, loss_g = sess.run([trip_loss_c, trip_loss_d, trip_loss_a, trip_loss_f, trip_loss_g], feed_dict={input_deltas: delta, 
                ...
                print('Step @ %d : norm: %.4f target loss: %.4f %.4f %.4f %.4f' % (i, np.mean(LA.norm(delta[0]*128, axis=-1)), loss_c, loss_a, loss_f, loss_g))
                #if(prediction_loss_c[0] + prediction_loss_a[0] > prev_loss - 0.0001):
                #    lost = True
                #    break
                # range : 2 ~ 4
                stop_condition = ((loss_c < -1.5 and loss_a < -1.5 and loss_f < -1.5) \
                        or (loss_f+loss_c+loss_a > prev_loss_f+prev_loss_c+prev_loss_a-0.0001))
                prev_loss_c = loss_c
                prev_loss_a = loss_a
                prev_loss_f = loss_f
                prev_loss_g = loss_g
                ...
                step_size = 0.5 #search_best_step_size(i, steps)
