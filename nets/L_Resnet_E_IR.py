...
def get_resnet(inputs, num_layers, scope=None, type=None, w_init=None, trainable=None, sess=None, reuse=False, keep_rate=None):
    if type == 'ir':
...
if __name__ == '__main__':
        ...
        # test resnetse
        nets = get_resnet(x, 50, scope='resnet_1950000_50', type='ir', w_init=w_init, trainable=False, keep_rate=dropout_rate)
        ...
        sess = tf.Session() 
        saver = tf.train.Saver([v for v in tf.all_variables() if '1950000' in v.name])
        saver.restore(sess, '../models/ckpt_model_c/InsightFace_iter_best_1950000.ckpt')
