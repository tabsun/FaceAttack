...
def resnet(inputs, bottle_neck, blocks, w_init=None, keep_rate=None, trainable=None, scope=None):
...
def get_resnet_gbn(inputs, num_layers, scope=None, keep_rate=None, type=None, w_init=None, trainable=None, sess=None):
    if type == 'ir':
...
if __name__ == '__main__':
        ...
        # test resnetse
        nets = get_resnet_gbn(x, 50, type='ir', keep_rate=dropout_rate, w_init=w_init, trainable=False, scope='resnet_1060000_50')
        ...
        saver.restore(sess, '../models/ckpt_model_a/InsightFace_iter_1060000.ckpt')

        embedding = nets.outputs
        emd = embedding.eval({x: [np.zeros((112,112,3),dtype=np.float)], dropout_rate:1.0}, session=sess)
        print(emd.shape)
